...
<head>
  ...
  <title>Writer Registration - School Publication</title>
</head>
...
<body class="flex flex-col min-h-screen">
  ...
  <div class="flex-grow flex items-center justify-center px-4">
    <div class="w-full max-w-md p-8 space-y-6 bg-white/90 rounded-3xl shadow-lg backdrop-blur relative">
      <!-- Heading -->
      <h2 class="text-2xl font-bold text-center text-primary">Create Writer Account</h2>
      ...
      <form action="core/handleForms.php" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <input type="text" name="username" id="username" class="w-full px-4 py-2 mt-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary" required>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="password" name="confirm_password" id="confirm_password" class="w-full px-4 py-2 mt-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary" required>
        </div>
        <div>
          <button type="submit" name="insertNewUserBtn" class="w-full bg-primary text-white px-5 py-3 rounded-2xl font-semibold text-sm shadow hover:bg-cyan-700">Register</button>
        </div>
      </form>

      <!-- Bottom links -->
      <div class="flex justify-between text-sm text-gray-600 mt-4">
        <a href="login.php" class="font-medium text-green-600 hover:underline">Writer Login</a>
        <a href="../admin/register.php" class="font-medium text-accent hover:underline">Admin Register</a>
      </div>
    </div>
  </div>
